...
model.eval()
with h5py.File(r"SPOT_1D_Train.h5", "a") as f:
...
with h5py.File(r"SPOT_1D_Valid.h5", "a") as f:
    for i in tqdm(range(n_SPOT_1D_Valid)):
        ...
        ESM_embedding=token_representations[0][1:len(seq) + 1].numpy()
        f.create_dataset('ESM2_650M'+id, data=ESM_embedding)
with h5py.File(r"Test2016.h5", "a") as f:
    for i in tqdm(range(n_Test2016)):
        ...
        ESM_embedding=token_representations[0][1:len(seq) + 1].numpy()
        f.create_dataset('ESM2_650M'+id, data=ESM_embedding)
with h5py.File(r"Test2018.h5", "a") as f:
    for i in tqdm(range(n_Test2018)):
        ...
        ESM_embedding=token_representations[0][1:len(seq) + 1].numpy()
        f.create_dataset('ESM2_650M'+id, data=ESM_embedding)
with h5py.File(r"Test2018.h5", "r") as f1:
    for i in tqdm(range(n_Test2018)):
        id=str(i)
        seq=f1['PS'+id][()]
        # print(str(seq)[2:-1])
        f = open(r'Test2018/ID'+id+'.fasta', 'w')
        f.write('>'+id+'\n')
        f.write(str(seq)[2:-1])
        f.close()
        os.system('cd Test2018'+';ASAquick '+'Test2018/ID'+id+'.fasta')
with h5py.File(r"SPOT_1D_Valid.h5", "a") as f1:
    for i in tqdm(range(n_SPOT_1D_Valid)):
        id=str(i)
        seq=f1['PS'+id][()]
        file = open(r'SPOT_1D_Valid/asaq.ID'+id+'.fasta'+'/rasaq.pred','r')
